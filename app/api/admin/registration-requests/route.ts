...
export async function POST(request: Request) {
  try {
    ...
    if (existingUser) {
      return NextResponse.json({ error: 'Email này đã được cấp quyền' }, { status: 409 })
    }
    ...
    if (existingRequest) {
      if (existingRequest.status === 'pending') {
        return NextResponse.json({ error: 'Yêu cầu đăng ký đang chờ xử lý' }, { status: 409 })
      }
      if (existingRequest.status === 'approved') {
        return NextResponse.json({ error: 'Email này đã được phê duyệt' }, { status: 409 })
      }
    }
